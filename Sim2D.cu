...
void Sim2D::ResetTerrain(int type)
{
    ...
    // Reflective walls on all 4 borders (high terrain).
...
void Sim2D::ResetWater(int type, float level)
{
    ...
    // Zero water on the four border rings to enforce the wall.
    for (int i = 0; i < GRIDRESOLUTION_2D; i++) {
        h   [i] = 0.f;
        hbar[i] = 0.f;
...
void Sim2D::EditWaterLocal(float xN, float yN, float size, float factor)
{
    ...
    // Re-enforce the wall on all 4 borders.
    for (int i = 0; i < GRIDRESOLUTION_2D; i++) {
        h[i]                                                   = 0.f;
        h[(GRIDRESOLUTION_2D - 1) * GRIDRESOLUTION_2D + i]     = 0.f;
    }
...
void Sim2D::SimStep(bool SWEonly)
{
    ...
    // ------------------------------------------------------------------
    kernel_recombine_qx<<<kGrid, kBlock>>>(qbarx_d, qtildex_d, h_d, terrain_d, qx_d);
    CUDA_CHECK_LAST();
    kernel_recombine_qy<<<kGrid, kBlock>>>(qbary_d, qtildey_d, h_d, terrain_d, qy_d);
    CUDA_CHECK_LAST();
